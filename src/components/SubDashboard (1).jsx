...
import {
  ...
  Banknote, Timer, LogOut, Mail, Phone, User, Shield,
  ChevronLeft, Star
} from "lucide-react";
...
function LoginScreen({onLogin}){
  ...
  const handleGooglePress=()=>{
    setLoading(true);
    // Simulate auth — in production this calls NextAuth signIn("google")
    setTimeout(()=>{setLoading(false);onLogin({isNew:true,email:"teacher@gmail.com"});},1400);
  };
  return(
    <div style={{width:"100%",maxWidth:430,minHeight:"100dvh",background:T.bg,display:"flex",flexDirection:"column",fontFamily:SF,position:"relative",overflow:"hidden"}}>
      {/* Background decoration */}
      <div style={{position:"absolute",top:-120,right:-80,width:320,height:320,borderRadius:"50%",background:"linear-gradient(135deg,rgba(0,122,255,0.10),rgba(88,86,214,0.08))",pointerEvents:"none"}}/>
      <div style={{position:"absolute",bottom:80,left:-60,width:220,height:220,borderRadius:"50%",background:"linear-gradient(135deg,rgba(52,199,89,0.07),rgba(0,122,255,0.05))",pointerEvents:"none"}}/>

      {/* Top illustration area */}
      <div style={{flex:1,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:"0 32px",paddingTop:60}}>
        {/* App icon */}
        <div style={{width:96,height:96,borderRadius:26,background:"linear-gradient(145deg,#007AFF,#5856D6)",display:"flex",alignItems:"center",justifyContent:"center",boxShadow:"0 16px 48px rgba(0,122,255,0.36)",marginBottom:28}}>
          <CalendarDays size={46} color="#fff" strokeWidth={1.6}/>
        </div>
        <h1 style={{fontSize:30,fontWeight:800,color:T.label,letterSpacing:"-1px",lineHeight:1.1,textAlign:"center",marginBottom:10,fontFamily:SFR}}>SubTrack</h1>
        <p style={{fontSize:16,color:T.label3,textAlign:"center",lineHeight:1.5,fontWeight:400,marginBottom:0}}>Your substitute teaching schedule,{"\n"}earnings, and schools — all in one place.</p>

        {/* Feature pills */}
        <div style={{display:"flex",gap:8,marginTop:28,flexWrap:"wrap",justifyContent:"center"}}>
          {[{icon:<CalendarCheck size={13} color="#34C759" strokeWidth={2}/>,label:"Schedule tracker"},{icon:<DollarSign size={13} color="#007AFF" strokeWidth={2}/>,label:"Earnings log"},{icon:<School size={13} color="#5856D6" strokeWidth={2}/>,label:"School manager"}].map(f=>(
            <div key={f.label} style={{display:"flex",alignItems:"center",gap:6,background:T.card,borderRadius:20,padding:"7px 14px",boxShadow:T.sh}}>
              {f.icon}
              <span style={{fontSize:13,fontWeight:500,color:T.label2}}>{f.label}</span>
            </div>
          ))}
        </div>
      </div>
      ...
      <div style={{padding:"0 20px 48px"}}>
        <button
          ...
          disabled={loading}
          style={{width:"100%",height:56,borderRadius:T.r8,border:"none",background:loading?"#E5E5EA":T.card,cursor:loading?"default":"pointer",display:"flex",alignItems:"center",justifyContent:"center",gap:12,boxShadow:loading?"none":T.shMd,transform:pressed?"scale(0.97)":"scale(1)",transition:"transform 0.15s cubic-bezier(.34,1.56,.64,1),box-shadow 0.15s",WebkitTapHighlightColor:"transparent",fontFamily:SF}}>
          {loading
            ? <><div style={{width:20,height:20,borderRadius:"50%",border:"2.5px solid #AEAEB2",borderTopColor:T.blue,animation:"spin 0.7s linear infinite"}}/><span style={{fontSize:16,fontWeight:600,color:T.label3}}>Signing in…</span></>
            : <><svg width="22" height="22" viewBox="0 0 24 24"><path d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z" fill="#4285F4"/><path d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" fill="#34A853"/><path d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l3.66-2.84z" fill="#FBBC05"/><path d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z" fill="#EA4335"/></svg>
              <span style={{fontSize:16,fontWeight:600,color:T.label}}>Continue with Google</span></>
          }
        </button>
        <style>{`@keyframes spin{to{transform:rotate(360deg)}}`}</style>
        <p style={{fontSize:12,color:T.label4,textAlign:"center",marginTop:16,lineHeight:1.5}}>By continuing, you agree to our Terms of Service and Privacy Policy. Your data is private and only visible to you.</p>
      </div>
    </div>
  );
}
...
function OnboardingScreen({email,onComplete}){
  ...
  const handleContinue=()=>{if(!canContinue)return;onComplete({name:name.trim(),email});};
  return(
    <div style={{width:"100%",maxWidth:430,minHeight:"100dvh",background:T.bg,display:"flex",flexDirection:"column",fontFamily:SF}}>
      <div style={{height:"env(safe-area-inset-top,44px)"}}/>{/* Safe area */}
      {/* Header */}
      <div style={{padding:"16px 20px 0",display:"flex",alignItems:"center"}}>
        <div style={{width:40,height:40,borderRadius:"50%",background:"linear-gradient(135deg,#007AFF,#5856D6)",display:"flex",alignItems:"center",justifyContent:"center"}}>
          <CalendarDays size={20} color="#fff" strokeWidth={1.8}/>
        </div>
        <div style={{marginLeft:12}}>
          <p style={{fontSize:11,fontWeight:600,color:T.label3,letterSpacing:"0.4px",textTransform:"uppercase"}}>Step 1 of 1</p>
          <p style={{fontSize:16,fontWeight:700,color:T.label,letterSpacing:"-0.3px"}}>Set up your profile</p>
        </div>
        {/* Progress */}
        <div style={{marginLeft:"auto",display:"flex",gap:5}}>
          <div style={{width:28,height:4,borderRadius:2,background:T.blue}}/>
        </div>
      </div>
      ...
      <div style={{flex:1,padding:"32px 24px 0"}}>
        ...
        <div style={{display:"flex",flexDirection:"column",alignItems:"center",marginBottom:36}}>
          <div style={{width:88,height:88,borderRadius:"50%",background:"linear-gradient(135deg,rgba(0,122,255,0.12),rgba(88,86,214,0.12))",border:`2px dashed ${T.blue}44`,display:"flex",alignItems:"center",justifyContent:"center",marginBottom:12}}>
            <User size={36} color={T.blue} strokeWidth={1.4}/>
          </div>
          <p style={{fontSize:13,color:T.label3,fontWeight:500}}>{email}</p>
        </div>

        <p style={{fontSize:22,fontWeight:800,color:T.label,letterSpacing:"-0.6px",marginBottom:6,fontFamily:SFR}}>What's your name?</p>
        <p style={{fontSize:15,color:T.label3,marginBottom:28,lineHeight:1.5}}>This is how you'll appear in the app. You can change it later in your profile.</p>

        {/* Name input */}
        <div style={{background:T.card,borderRadius:T.r8,overflow:"hidden",boxShadow:T.sh,marginBottom:12}}>
          <div style={{display:"flex",alignItems:"center",padding:"0 16px",height:56,gap:12}}>
            <User size={18} color={T.label4} strokeWidth={1.8}/>
            <input
              ref={inputRef}
              value={name}
              ...
              style={{flex:1,border:"none",outline:"none",fontSize:16,color:T.label,background:"transparent",fontFamily:SF,letterSpacing:"-0.2px"}}
            />
            {name.length>=2&&<CheckCircle size={18} color={T.green} strokeWidth={2.2}/>}
          </div>
        </div>
        <p style={{fontSize:12,color:T.label4,marginBottom:0,paddingLeft:4}}>This name will appear as your greeting on the dashboard.</p>
      </div>
      ...
      <div style={{padding:"20px 24px 48px"}}>
        <button
          ...
          style={{width:"100%",height:56,borderRadius:T.r8,border:"none",background:canContinue?"linear-gradient(135deg,#007AFF,#5856D6)":"rgba(120,120,128,0.18)",color:"#fff",fontWeight:700,fontSize:17,cursor:canContinue?"pointer":"default",fontFamily:SF,boxShadow:canContinue?"0 6px 24px rgba(0,122,255,0.40)":"none",transform:pressed&&canContinue?"scale(0.97)":"scale(1)",transition:"transform 0.15s cubic-bezier(.34,1.56,.64,1),background 0.2s",WebkitTapHighlightColor:"transparent",display:"flex",alignItems:"center",justifyContent:"center",gap:8}}>
          {canContinue?"Get started →":"Enter your name to continue"}
        </button>
      </div>
    </div>
  );
...
function ProfileSheet({user,onClose,onLogout}){
  ...
  useEffect(()=>{const t=setTimeout(()=>setVisible(true),20);return()=>clearTimeout(t);},[]);
  const handleClose=()=>{setVisible(false);setTimeout(onClose,340);};
  const handleLogout=()=>{setVisible(false);setTimeout(()=>{onLogout();},340);};
  const initials=user.name.split(" ").map(n=>n[0]).join("").toUpperCase().slice(0,2);
  return(
    <div style={{position:"fixed",inset:0,zIndex:400,background:visible?"rgba(0,0,0,0.5)":"rgba(0,0,0,0)",backdropFilter:visible?"blur(20px) saturate(180%)":"blur(0px)",WebkitBackdropFilter:visible?"blur(20px) saturate(180%)":"blur(0px)",transition:"background 0.34s ease,backdrop-filter 0.34s ease",display:"flex",alignItems:"flex-end",justifyContent:"center",fontFamily:SF}} onClick={handleClose}>
      <div onClick={e=>e.stopPropagation()} style={{width:"100%",maxWidth:430,background:T.bg,borderRadius:"32px 32px 0 0",paddingBottom:34,transform:visible?"translateY(0)":"translateY(100%)",transition:"transform 0.4s cubic-bezier(0.32,0.72,0,1)",boxShadow:"0 -2px 60px rgba(0,0,0,0.22)",maxHeight:"92dvh",overflowY:"auto"}}>
        {/* Handle */}
        <div style={{display:"flex",justifyContent:"center",paddingTop:14,paddingBottom:4}}><div style={{width:36,height:5,borderRadius:3,background:T.fill3}}/></div>

        {/* Profile hero */}
        <div style={{padding:"16px 24px 20px",display:"flex",alignItems:"center",gap:16}}>
          <div style={{width:64,height:64,borderRadius:"50%",background:"linear-gradient(135deg,#007AFF,#5856D6)",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0,boxShadow:"0 6px 20px rgba(0,122,255,0.32)"}}>
            <span style={{fontSize:22,fontWeight:800,color:"#fff",fontFamily:SFR}}>{initials}</span>
          </div>
          <div style={{flex:1,minWidth:0}}>
            <p style={{fontSize:20,fontWeight:800,color:T.label,letterSpacing:"-0.5px",fontFamily:SFR}}>{user.name}</p>
            <p style={{fontSize:13,color:T.label3,marginTop:2,display:"flex",alignItems:"center",gap:4}}>
              <Mail size={12} color={T.label4} strokeWidth={2}/>{user.email}
            </p>
          </div>
        </div>
        ...
        <div style={{margin:"0 16px",display:"flex",flexDirection:"column",gap:10}}>
          {/* Account info */}
          <div style={{background:T.card,borderRadius:T.r8,overflow:"hidden",boxShadow:T.sh}}>
            <div style={{display:"flex",alignItems:"center",padding:"14px 16px",gap:12,borderBottom:`0.5px solid ${T.sep}`}}>
              <div style={{width:34,height:34,borderRadius:10,background:"#EEF3FF",display:"flex",alignItems:"center",justifyContent:"center"}}><User size={17} color="#3B6FE0" strokeWidth={1.8}/></div>
              <div style={{flex:1}}><p style={{fontSize:13,color:T.label3,fontWeight:500,marginBottom:1}}>Name</p><p style={{fontSize:15,fontWeight:600,color:T.label}}>{user.name}</p></div>
            </div>
            <div style={{display:"flex",alignItems:"center",padding:"14px 16px",gap:12}}>
              <div style={{width:34,height:34,borderRadius:10,background:"#EDFAF3",display:"flex",alignItems:"center",justifyContent:"center"}}><Mail size={17} color="#34C759" strokeWidth={1.8}/></div>
              <div style={{flex:1}}><p style={{fontSize:13,color:T.label3,fontWeight:500,marginBottom:1}}>Email</p><p style={{fontSize:15,fontWeight:600,color:T.label}}>{user.email}</p></div>
            </div>
          </div>
          ...
          <div style={{background:T.card,borderRadius:T.r8,overflow:"hidden",boxShadow:T.sh}}>
            <div style={{display:"flex",alignItems:"center",padding:"14px 16px",gap:12}}>
              <div style={{width:34,height:34,borderRadius:10,background:"#F3EEFF",display:"flex",alignItems:"center",justifyContent:"center"}}><Shield size={17} color="#5856D6" strokeWidth={1.8}/></div>
              <div style={{flex:1}}><p style={{fontSize:15,fontWeight:600,color:T.label}}>Your data is private</p><p style={{fontSize:12,color:T.label3,marginTop:1}}>Only you can see your schedule and earnings</p></div>
            </div>
          </div>
          ...
          <div style={{background:T.card,borderRadius:T.r8,overflow:"hidden",boxShadow:T.sh}}>
            {!showLogoutConfirm
              ? <button onClick={()=>setShowLogoutConfirm(true)} style={{width:"100%",display:"flex",alignItems:"center",padding:"14px 16px",gap:12,border:"none",background:"transparent",cursor:"pointer",fontFamily:SF,WebkitTapHighlightColor:"transparent"}}>
                  <div style={{width:34,height:34,borderRadius:10,background:"#FFF0F0",display:"flex",alignItems:"center",justifyContent:"center"}}><LogOut size={17} color={T.red} strokeWidth={1.8}/></div>
                  <p style={{fontSize:15,fontWeight:600,color:T.red,flex:1,textAlign:"left"}}>Sign out</p>
                  <ChevronRight size={16} color={T.label4} strokeWidth={2}/>
                </button>
              : <div style={{padding:"14px 16px"}}>
                  <p style={{fontSize:14,fontWeight:600,color:T.label,marginBottom:12}}>Sign out of SubTrack?</p>
                  <div style={{display:"flex",gap:10}}>
                    <button onClick={()=>setShowLogoutConfirm(false)} style={{flex:1,height:42,borderRadius:12,border:"none",background:T.fill4,color:T.label,fontWeight:600,fontSize:14,cursor:"pointer",fontFamily:SF,WebkitTapHighlightColor:"transparent"}}>Cancel</button>
                    <button onClick={handleLogout} style={{flex:1,height:42,borderRadius:12,border:"none",background:T.red,color:"#fff",fontWeight:700,fontSize:14,cursor:"pointer",fontFamily:SF,boxShadow:"0 4px 16px rgba(255,59,48,0.32)",WebkitTapHighlightColor:"transparent"}}>Sign out</button>
                  </div>
                </div>
            }
          </div>
          ...
          {/* App version */}
          <p style={{textAlign:"center",fontSize:12,color:T.label4,padding:"4px 0 8px"}}>SubTrack v1.0 · Made for substitute teachers</p>
        </div>
      </div>
    </div>
  );
...
export default function App() {
  // Auth state — null=logged out, {isNew,email}=just authed, {name,email}=fully onboarded
  const [authStep,  setAuthStep]  =useState("login"); // "login" | "onboard" | "app"
  const [user,      setUser]      =useState(null);
  const [showProfile,setShowProfile]=useState(false);

  const handleLogin=(data)=>{
    if(data.isNew){ setUser({email:data.email}); setAuthStep("onboard"); }
    else           { setUser(data);               setAuthStep("app"); }
  };
  const handleOnboardComplete=(data)=>{ setUser(data); setAuthStep("app"); };
  const handleLogout=()=>{ setUser(null); setAuthStep("login"); setShowProfile(false); };
  ...
  // ── Auth gates ─────────────────────────────────────────
  if(authStep==="login"){
    return(<><style>{globalStyles}</style><div style={{display:"flex",justifyContent:"center",minHeight:"100dvh",background:"#E5E5EA"}}><LoginScreen onLogin={handleLogin}/></div></>);
  }
  if(authStep==="onboard"){
    return(<><style>{globalStyles}</style><div style={{display:"flex",justifyContent:"center",minHeight:"100dvh",background:"#E5E5EA"}}><OnboardingScreen email={user?.email??""} onComplete={handleOnboardComplete}/></div></>);
  }
        ...
        <div style={{width:"100%",maxWidth:430,background:"#F2F2F7",minHeight:"100dvh",overflowX:"hidden",position:"relative",fontFamily:SF}}>
          ...
          <div style={{padding:"10px 20px 8px",display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
            <div>
              ...
              <span style={{fontSize:36,fontWeight:800,color:T.label,letterSpacing:"-1.2px",lineHeight:1.05,fontFamily:SFR}}>Hi {user?.name?.split(" ")[0]??"there"} 👋</span>
            </div>
            <button onClick={()=>setShowProfile(true)} style={{width:40,height:40,borderRadius:"50%",
              background:"linear-gradient(135deg,#007AFF,#5856D6)",border:"none",
              cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",
              boxShadow:"0 4px 16px rgba(0,122,255,0.36)",marginTop:4,
              WebkitTapHighlightColor:"transparent"}}>
              <span style={{fontSize:14,fontWeight:800,color:"#fff",fontFamily:SFR}}>
